...
const FloristBoard: React.FC<FloristBoardProps> = ({
  orders,
  onUpdateStatus,
  products = [],
}) => {
  ...
  const [expandedRecipeOrderId, setExpandedRecipeOrderId] = useState<
    string | null
  >(null);
  ...
  const toggleRecipe = (orderId: string) => {
    setExpandedRecipeOrderId(
      expandedRecipeOrderId === orderId ? null : orderId
    );
  };
    ...
    <div className="h-full flex flex-col space-y-8 animate-fade-in">
      ...
      {previewImage && (
        ...
        >
          ...
          >
            ...
            />
            <p className="mt-4 text-white font-bold text-lg px-6 py-2 bg-primary/20 rounded-full backdrop-blur-sm border border-white/10">
              Xem chi tiết mẫu cắm
            </p>
          </div>
        </div>
      )}
      ...
      <div className="flex-none h-[calc(100vh-350px)] flex flex-col">
        <div className="flex justify-between items-end mb-6">
          <div>
            ...
            <p className="text-slate-500 font-medium">
              Bảng phân việc thợ cắm hoa theo thời gian giao
            </p>
          </div>
        ...
        <div className="flex-1 flex gap-6 overflow-x-auto pb-4 no-scrollbar">
          {columns.map((col) => (
            <div
              key={col.id}
              className={`flex-shrink-0 w-[400px] ${col.color} rounded-[2rem] p-5 flex flex-col h-full border border-slate-200/60 shadow-sm`}
            >
                    ...
                    >
                      ...
                      {order.referenceImageUrl && (
                        <div className="relative h-48 w-full bg-slate-100 overflow-hidden group/img">
                          ...
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              setPreviewImage(order.referenceImageUrl!);
                            }}
                          ...
                          <div
                            className={`absolute top-4 right-4 z-10 px-3 py-1.5 rounded-xl font-black text-xs flex items-center gap-1.5 shadow-lg border-2 ${
                              isOverdue
                      ...
                      <div className="p-5 flex-1 flex flex-col gap-4">
                        ...
                        <div className="flex justify-between items-start">
                          <div>
                            <span className="text-[10px] font-mono font-black text-slate-400 bg-slate-50 px-2 py-0.5 rounded-lg border">
                              #{order.id}
                            </span>
                            <h4 className="font-black text-xl text-slate-800 mt-2 tracking-tight line-clamp-1">
                              {order.occasion || "Mẫu thiết kế"}
                            </h4>
                          </div>
                        </div>
                        ...
                        <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
                          ...
                          <div className="space-y-2">
                            {order.items.map((item, idx) => (
                              <div
                                key={idx}
                                className="text-sm text-slate-700 flex justify-between items-center bg-white p-2.5 rounded-xl border border-slate-100 shadow-sm"
                              >
                                <span className="font-bold">
                                  {item.productName}
                                </span>
                                <span className="bg-slate-900 text-white w-7 h-7 flex items-center justify-center rounded-lg text-xs font-black">
                                  x{item.quantity}
                                </span>
                              </div>
                            ))}
                            ...
                            <div>
                              ...
                              >
                                <span className="flex items-center gap-2">
                                  <ScrollText size={16} /> Nguyên liệu cần chuẩn
                                  bị
                                </span>
                                {isExpanded ? (
                                  <ChevronUp size={16} />
                                    ...
                                    {materials.map((mat, idx) => (
                                      ...
                                      >
                                        ...
                                        <span className="font-black text-slate-900 bg-slate-50 px-2 py-0.5 rounded-lg border">
                                          {parseFloat(mat.quantity.toFixed(1))}{" "}
                                          {mat.unit}
                                        </span>
                                      </li>
                                    ))}
                        ...
                        {order.note && (
                          <div className="text-[11px] p-3 bg-amber-50 text-amber-800 border border-amber-200 rounded-xl font-medium">
                            <strong className="font-black uppercase tracking-tighter">
                              Ghi chú in thiệp:
                            </strong>{" "}
                            {order.note}
                          </div>
                            ...
                            <button
                              ...
                              className={`w-full py-4 text-white text-sm font-black rounded-2xl transition-all shadow-xl active:scale-95 uppercase tracking-widest ${
                                isOverdue
                                  ...
                                  : "bg-primary hover:bg-pink-700 shadow-pink-200"
                              }`}
                            >
                              Bắt đầu ngay
                            </button>
      ...
      <div className="bg-white rounded-[2rem] shadow-sm border border-slate-200 p-8 flex-1 min-h-[300px]">
        <div className="flex items-center justify-between mb-8">
          ...
          <button className="text-sm font-black text-primary hover:underline uppercase tracking-widest">
            Xem toàn bộ lịch sử
          </button>
        </div>
          ...
          <table className="w-full text-left">
            <thead className="bg-slate-50 text-[10px] font-black uppercase text-slate-400 tracking-widest">
              <tr>
                ...
                <th className="px-6 py-5 rounded-r-2xl text-center">
                  Trạng thái
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 text-sm">
                ...
                >
                    ...
                    <div className="flex flex-col gap-1">
                      {order.items.map((item, idx) => (
                        <div key={idx} className="text-slate-600 font-medium">
                          {item.productName}{" "}
                          <span className="font-black text-slate-400 ml-1 text-xs">
                            x{item.quantity}
                          </span>
                        </div>
                      ))}
                  ...
                  <td className="px-6 py-5 text-center">
                    {order.completedImageUrl ? (
                      <button
                        onClick={() =>
                          setPreviewImage(order.completedImageUrl || null)
                        }
              ...
              {historyOrders.length === 0 && (
                <tr>
                  <td
                    colSpan={5}
                    className="text-center py-12 text-slate-400 font-black uppercase tracking-widest opacity-20"
                  >
                    Chưa có dữ liệu
                  </td>
                </tr>
